...
jQuery.validator.addMethod("mac", function(value, element) {
	return this.optional(element) || (/[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}-[A-Fa-f0-9]{2}$/.test(value));   
}, "请输入合法的MAC地址,如：Aa-Aa-33-Aa-Aa-Aa");
...
jQuery.validator.addMethod("phone", function(value, element) {
	var tel = /(^((0\d{2,3})-)(\d{7,8})$)|(^1[3,4,7,5,8]\d{9}$)|(^(0\d{2,3})(\d{7,8})$)/g;     
	return this.optional(element) || (tel.test(value));     
}, "区号(3-4位)号码(7-9位)或13,14,15,17,18号段");	
...
jQuery.validator.addMethod("num", function(value, element) {
    var tel = /^(([a-z]+[0-9]+)|([0-9]+[a-z]+))[a-z0-9]*$/i;
    return this.optional(element) || (tel.test(value));
}, "请正确填写数字");
...
//校验正数
jQuery.validator.addMethod("nofs",function(value, element) {
	var tel = /^\d+(\.\d+)?$/;
	return this.optional(element) || (tel.test(value));
},"请输入正数");
